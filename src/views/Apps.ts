    ...
    apiClient.getOpenInApps()
        .then(response => {
            ...
            if (view.listContainer) {
                if (apps.length === 0) {
                    view.listContainer.innerHTML = '<div style="color: var(--text-secondary);">No apps configured.</div>';
                    return;
                } else {
                    view.listContainer.innerHTML = apps.map((app: any) => {
                        ...
                        if (app.terminal) {
                            ...
                            badges.push(`<span class="badge" style="background: rgba(20, 184, 166, 0.12); border: 1px solid rgba(20, 184, 166, 0.35); color: #2dd4bf;" title="Terminal App">terminal</span>`);
                            if (isDefault) {
                                badges.push(`<span class="badge" style="background: rgba(245, 158, 11, 0.15); border: 1px solid rgba(245, 158, 11, 0.4); color: #fbbf24;" title="Default Terminal">★ default</span>`);
                            }
                        }
                        return `
                        <div class="list-item">
                            <div>
                                ...
                                <div class="list-item-meta">${app.id} · ${app.enabled !== false ? 'enabled' : 'disabled'}</div>
                            </div>
                        </div>
                    `}).join('');
                }
